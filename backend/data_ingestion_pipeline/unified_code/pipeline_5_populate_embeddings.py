    ...
    with driver.session(database="neo4j") as session:
        ...
        for label in LABELS_TO_EMBED:
            count_result = session.run(f"""
                MATCH (n:{label})
                WHERE n.embedding IS NULL AND (n.text IS NOT NULL OR n.description IS NOT NULL OR n.rows IS NOT NULL)
                RETURN count(n) as count
            """).single()
            
            count = count_result['count'] if count_result else 0
            total_nodes_to_embed += count
            logging.info(f"   - Found {count} nodes with label ':{label}' needing an embedding.")
